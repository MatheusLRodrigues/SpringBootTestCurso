...
//@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT) -- Used to define a specific port for you test
...
//    }
    ...
    void testCreateUser_whenValidDetailsProvided_returnUserDetails() {
        //Arrange
        ...
        assertEquals(userDetailsRequestJson.getAsString("firstName"), createdUserDetails.getFirstName(), "Returned user's first name seems to be incorrect");
        assertEquals(userDetailsRequestJson.getAsString("lastName"), createdUserDetails.getLastName(), "Returned user's last name seems to be incorrect");
        assertEquals(userDetailsRequestJson.getAsString("email"), createdUserDetails.getEmail(), "Returned user's email seems to be incorrect");
        assertFalse(createdUserDetails.getUserId().trim().isEmpty(), "User ID should not be empty");
    }

    @Order(2)
    @Test
    @DisplayName("GET /users request JWT")
    void testGetUsers_whenMissingJWT_returns403() {
        ...
        //Assert
        assertEquals(HttpStatus.FORBIDDEN, responseEntity.getStatusCode(), "HTTP status code 403 forbidden should have been returned");
    }
    ...
    void testGetUsers_whenValidJWWTProvided_returnUsers() {
        //Arrange
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(List.of(MediaType.APPLICATION_JSON));
        ...
        assertEquals(1, responseEntity.getBody().size(), "There should be exactly 1 user in the list");
    }
}
